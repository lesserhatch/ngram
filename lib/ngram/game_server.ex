defmodule Ngram.GameServer do
  ...
  def buy_vowel(game_code, player_id, letter) do
    GenServer.call(via_tuple(game_code), {:buy_vowel, player_id, letter})
  end
  ...
  @impl true
  def handle_call({:buy_vowel, player_id, letter}, _from, %GameState{} = state) do
    with {:ok, player} <- GameState.find_player(state, player_id),
         {:ok, new_state} <- GameState.buy_vowel(state, player, letter) do
      broadcast_game_state(new_state)
      {:reply, :ok, new_state}
    else
      {:error, reason} = error ->
  ...
  def broadcast_game_state(%GameState{} = state) do
    PubSub.broadcast(Ngram.PubSub, "game:#{state.code}", {:game_state, state})
  end
  ...
  GameServer.
  """
  def via_tuple(game_code), do: {:via, Horde.Registry, {Ngram.GameRegistry, game_code}}
  ...
  def server_found?(game_code) do
    ...
    case Horde.Registry.lookup(Ngram.GameRegistry, game_code) do
      [] -> false
